...
def draw_nrcc_messages(draw: ImageDraw.ImageDraw, nrcc_messages: list) -> None:
    offset: int = 87
    for count, nrcc_message in enumerate(nrcc_messages):
        message: str = nrcc_message["value"]
        sanitized: str = bleach.clean(message, tags=[], strip=True).strip()
        lines = get_multiline_text(
            sanitized, Font.DOTMATRIX, Display.WIDTH - (Display.MARGIN * 2)
        )
        for number, line in enumerate(lines):
            y = offset + 38 + (number * 38)
            draw.text(
                (Display.WIDTH / 2, y), line.strip(), Color.YELLOW, Font.DOTMATRIX, "mb"
            )
        if count == 0:
            break
    ...
    if services["trainServices"] is not None:
        ...
        for service in services["trainServices"]:
            ...
            if service["std"] is not None:
                draw.text(
                    (Display.LEFT, offset), service["std"], Color.YELLOW, Font.DOTMATRIX
                )
            ...
            if service["etd"] is not None:
                etd: str = service["etd"]
                draw.text(
                    xy=(Display.WIDTH - Display.MARGIN, offset),
                    text=etd,
                    fill=Color.YELLOW,
                    font=Font.DOTMATRIX,
                    anchor="rt",
        ...
        if services["nrccMessages"] is not None:
...
def get_departures(crs: str) -> None:
    """Display plain-text table of upcoming departures from a named station."""
    services = get_train_services(crs=crs, rows=9, expand=True)
